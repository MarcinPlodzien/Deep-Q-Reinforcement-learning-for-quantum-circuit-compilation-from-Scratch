...
class QuantumCircuitEnvironment:
    def __init__(self, L, psi_initial, psi_target,  max_steps, fidelity_threshold):
        ...
        self.action_space_entangling_parametrized_gates     = [(gate_symbol, i, phase) for gate_symbol in ["Id", "Rxx", "Ryy", "Rzz"] for i in get_range(1, L) for phase in self.phase_grid] 
...
class DQNAgent:
    def __init__(self, action_space, state_dim, learning_rate, gamma, epsilon):
    ...
    def learn(self, psi_current, action_idx, reward, psi_next, done):
        psi_current_Re_Im      = pt.concatenate([psi_current.real, psi_current.imag], dim=-1)
        psi_next_Re_Im         = pt.concatenate([psi_next.real, psi_next.imag], dim=-1)
         
        reward = pt.FloatTensor([reward])
        done = pt.FloatTensor([done])
        
        current_q = self.model(psi_current_Re_Im)[action_idx]
        max_next_q = self.model(psi_next_Re_Im).max()
...
psi_initial = ket["|00>"]                      # Initial state of quantum circuit
norm = pt.sum(pt.abs(psi_initial)**2)
psi_initial = psi_initial/pt.sqrt(norm)

psi_target = ket["|00>"] + ket["|11>"]     # Target state of quantum circuit
norm = pt.sum(pt.abs(psi_target)**2)
psi_target = psi_target/pt.sqrt(norm)

fidelity_threshold  = 0.99                      # 
max_steps           = 5                         # maximum number of agent steps
...
agent = DQNAgent(env.action_space, env.state_dimension, learning_rate, gamma, epsilon) 
